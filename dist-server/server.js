...
const oauth2Client = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, `${process.env.APP_URL}/api/auth/google/callback`);
...
async function startServer() {
    ...
    app.post("/api/content/:id/publish", (req, res) => {
        ...
        // Update streak
        const streakData = db.prepare('SELECT * FROM streaks WHERE user_id = ?').get(DEFAULT_USER_ID);
        if (!streakData) {
    ...
    app.get("/api/auth/google/callback", async (req, res) => {
        ...
        `).run(tokens.access_token, tokens.refresh_token || null, tokens.expiry_date, JSON.stringify(userInfo.data), DEFAULT_USER_ID, 'youtube');
            }
            else {
                db.prepare(`
          INSERT INTO user_accounts (user_id, platform, access_token, refresh_token, expiry_date, profile_data)
          VALUES (?, ?, ?, ?, ?, ?)
        `).run(DEFAULT_USER_ID, 'youtube', tokens.access_token, tokens.refresh_token, tokens.expiry_date, JSON.stringify(userInfo.data));
            }
            res.send(`
        <html>
    ...
    app.get("/api/analytics/youtube", async (req, res) => {
        const account = db.prepare('SELECT * FROM user_accounts WHERE user_id = ? AND platform = ?').get(DEFAULT_USER_ID, 'youtube');
        if (!account)
            return res.json({ views: 0, subscribers: 0 });
        try {
            const auth = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET);
            auth.setCredentials({
                access_token: account.access_token,
